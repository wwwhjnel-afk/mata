...
const PaymentUpdateModal: React.FC<PaymentUpdateModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    paymentStatus: trip.payment_status || 'unpaid' as 'unpaid' | 'partial' | 'paid',
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.paymentStatus !== 'unpaid') {
      ...
      } else if (trip.base_revenue && amount > trip.base_revenue) {
      ...
      if (!formData.paymentReceivedDate) {
        ...
        const invoiceDate = trip.invoice_date ? new Date(trip.invoice_date) : null;
  ...
  const isFullPayment = trip.base_revenue && Number(formData.paymentAmount) === trip.base_revenue;
  const isPartialPayment = trip.base_revenue && Number(formData.paymentAmount) > 0 && Number(formData.paymentAmount) < trip.base_revenue;
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Record payment receipts and update invoice payment status
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-md p-4">
            ...
            <div className="flex items-center space-x-3">
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border ${getPaymentStatusColor(trip.payment_status)}`}>
                {trip.payment_status?.toUpperCase() || 'UNPAID'}
              </span>
              {trip.payment_amount && (
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value: string) => handleChange('paymentStatus', value)}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      step="0.01"
                      min="0.01"
                      max={trip.base_revenue || undefined}
