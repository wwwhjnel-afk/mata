...
const InspectionTypeSelector = () => {
  ...
  const handleRoutineInspection = async () => {
    ...
    try {
      ...
      // Step 2: Create the inspection in vehicle_inspections table
      ...
      const { data: newInspection, error: inspectionError } = await supabase
        ...
        .insert({
          ...
          inspector_profile_id: inspectorId,
          ...
          scanned_vehicle_qr: scannedVehicleData?.fullCode || null,
          vehicle_registration: vehicleData.registration_number,
          vehicle_make: vehicleData.make || null,
          vehicle_model: vehicleData.model || null,
          odometer_reading: state.odometerReading || null,
        })
        .select()
        .single();
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <div className="sticky top-0 z-10 bg-card/95 backdrop-blur-md border-b border-border/50 safe-area-top">
      ...
      <div className="p-4 space-y-6 max-w-2xl mx-auto">
          ...
          <CardContent className="pt-6">
            <div className="space-y-2">
              ...
              {state.odometerReading && (
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-muted-foreground">Odometer</span>
                  <span className="font-semibold">{state.odometerReading.toLocaleString()} km</span>
                </div>
              )}
            </div>
          </CardContent>
        ...
        <div className="space-y-4">
          ...
          {/* Tyre Inspection */}
          <Card
            className={`cursor-pointer active:border-primary active:shadow-md transition-all rounded-xl ${isCreating ? 'opacity-50 pointer-events-none' : ''}`}
          ...
          >
            ...
            <CardContent>
              <Button className="w-full bg-orange-500 active:bg-orange-600" size="lg" disabled={isCreating}>
          ...
          <Card
            className={`cursor-pointer active:border-primary active:shadow-md transition-all rounded-xl ${isCreating ? 'opacity-50' : ''}`}
