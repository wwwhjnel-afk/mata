-- Migration: Add detailed workflow timestamp columns to loads table
...
-- Add new enum values to existing load_status enum
-- Note: PostgreSQL enums cannot be modified directly in a transaction,
-- so we add values one at a time using IF NOT EXISTS
DO $$
BEGIN
  -- Add new workflow statuses to the enum
  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'arrived_at_loading' AND enumtypid = 'load_status'::regtype) THEN
    ALTER TYPE load_status ADD VALUE 'arrived_at_loading';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'loading' AND enumtypid = 'load_status'::regtype) THEN
    ALTER TYPE load_status ADD VALUE 'loading';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'loading_completed' AND enumtypid = 'load_status'::regtype) THEN
    ALTER TYPE load_status ADD VALUE 'loading_completed';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'arrived_at_delivery' AND enumtypid = 'load_status'::regtype) THEN
    ALTER TYPE load_status ADD VALUE 'arrived_at_delivery';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'offloading' AND enumtypid = 'load_status'::regtype) THEN
    ALTER TYPE load_status ADD VALUE 'offloading';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'offloading_completed' AND enumtypid = 'load_status'::regtype) THEN
    ALTER TYPE load_status ADD VALUE 'offloading_completed';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'completed' AND enumtypid = 'load_status'::regtype) THEN
    ALTER TYPE load_status ADD VALUE 'completed';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'on_hold' AND enumtypid = 'load_status'::regtype) THEN
    ALTER TYPE load_status ADD VALUE 'on_hold';
  END IF;
END $$;-- Create index for status queries
